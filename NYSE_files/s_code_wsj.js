if (typeof dj == "undefined") { this.dj = {}; }
...
return editionToNameMap[getRegion.toLowerCase()];
  ...
  if((typeof gcDomain !== "undefined") && (gcDomain.search( "smartmoney.com" ) != -1)) {
    s_account = "djglobal,djsm";
  }

  if((typeof gcDomain !== "undefined") && (gcDomain.search( "marketwatch.com" ) != -1)) {
    s_account = "djglobal,djmarketwatch";
  }

  //adding reportsuite for forex blogs
  if(env.indexOf('dowjones.com') != -1){
    s_account = "djfxtrader";
 }

  if((typeof gcDomain !== "undefined") && gcDomain.search( "wsjcfo.dealogic.com" ) != -1 ) {
      s_account=s_account+"cfo"; 
  }
	...
	//Override reportsuite for Indo Referrer or Region
	var indonesiaUrlPattern = new RegExp("indo(.f.dev|.s.dev|).wsj.com");
    if (ref.match(indonesiaUrlPattern) || (currentRegion=="asia,indo")) {
	    if(s_account === "dowjdev"){
	    	s_account="dowjdev,djwsjindonesia";
	    }else{
	    	s_account="djglobal,djwsjindonesia";
	    }
    }	
	//Override report suite for Korea  Referrer or Region
    var koreaUrlPattern = new RegExp("kr(.f.dev|.s.dev|).wsj.com");
    if (ref.match(koreaUrlPattern) || (currentRegion=="asia,kr")) {
	    if(s_account === "dowjdev"){
	    	s_account="dowjdev,djwsjkorea";
	    }else{
...
function s_doPlugins(s) {
   ...
   * 
   */
  var qryPMod = s.getQueryParam('mod');
  var qryPFb_ref = s.getQueryParam('fb_ref');
  var qryPFb_src = s.getQueryParam('fb_source');
  var qryPLink = s.getQueryParam('link');
  var qryPReflink = s.getQueryParam('reflink');
  var qryPDistag = s.getQueryParam('distag');
  var qryPSiteid = s.getQueryParam('siteid');
  var qryPCid = s.getQueryParam('cid');
  
  if((document.location.href).indexOf('?')!==-1 && (document.location.href).indexOf('=')!==-1){
      if(qryPMod){
        s.campaign = qryPMod;
      }else if(qryPFb_ref){
        s.campaign = qryPFb_ref;
    if(qryPFb_src){
          s.campaign = qryPFb_ref +'_'+ qryPFb_src;
        }
      }else if(qryPLink){
        s.campaign = qryPLink;
      }else if(qryPReflink){
        s.campaign = qryPReflink;
  }else if(qryPDistag){
        s.campaign = qryPDistag;
      }else if(qryPSiteid){
        s.campaign = qryPSiteid;
      }else if(qryPCid){
        s.campaign = qryPCid;
      }
  }
  s.prop49=s.getVisitNum();
  s.eVar62=s.getFullReferringDomains(); //subdomain referrer
  /*
   * s.pageName is page name
   * cookie name is 'gpv_pn'
   * wsj page view event is 'event12'
   * 
   */
  s.splitPrevVal();
  s.prop65=s.getPreviousValue(s.pageName, 'gpv_pn', 'event12'); //previous page value
}
...
/*
* Plugin: Get Full Referring Domains (Subdomain Referrer)
*/ 
s.getFullReferringDomains=new Function(""
+"var s=this,dr=document.referrer,n=s.linkInternalFilters.spli"
...
* name changed to 'splitPrevVal' - to avoid confusion w/ default split
*/
s.splitPrevVal=new Function("l","d","" 
+"var i,x=0,a=new Array;while(l){i=l.indexOf(d);i=i>-1?i:l.length;a[x" 
+"++]=l.substring(0,i);l=l.substring(i+d.length);}return a"); 

/* 
* Plugin: getPreviousValue_v1.0 - return previous value of designated 
* variable (requires split utility i.e. splitPrevVal) 
*/ 
s.getPreviousValue=new Function("v","c","el","" 
+"var s=this,t=new Date,i,j,r='';t.setTime(t.getTime()+1800000);if(el" 
+"){if(s.events){i=s.splitPrevVal(el,',');j=s.splitPrevVal(s.events,',');for(x in i" 
